...
class GradeController extends Controller
{
        ...
        $data->where(function ($query) use ($request) {
            if ($request->search) {
                $query->orWhere("profile_id", 'LIKE', "%$request->search%")
                    ...
                    ->orWhere("school_year_id", 'LIKE', "%$request->search%")
     ...
     */
    public function grade_archived(Request $request)
    {
        $ret = [
            "success" => false,
        ...
        if ($request->ids && count($request->ids) > 0) {
            if ($request->isTrash) {
                Grade::whereIn("id", $request->ids)->restore();
                // Update the 'updated_by' field
                $data = [
                    "deleted_by" => null,
                    "updated_by" => auth()->id(),
                ];
                Grade::whereIn("id", $request->ids)->update($data);
            } else {
                $data = [
                    "deleted_at" => now(),
                    "deleted_by" => auth()->id(),
