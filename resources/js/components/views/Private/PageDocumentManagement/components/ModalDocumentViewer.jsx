...
export default function ModalDocumentViewer({ open, onClose, documentData, profileData, profilesData }) {
    ...
    React.useEffect(() => {
        if (open && profilesData) {
            // Component is ready to display document viewer
        }
    ...
    const handlePreviewDocument = (profile) => {
        ...
        setSelectedDocumentForPreview({
            ...
            // Preserve user role data from the original documentData
            vice_president_name: documentData?.vice_president_name,
            university_president_name: documentData?.university_president_name,
            dean_name: documentData?.dean_name,
