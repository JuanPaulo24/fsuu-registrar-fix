...
import TabDocumentTrackingsContext from "./TabDocumentTrackingsContext";
import ModalDocumentHistory from "./ModalDocumentHistory";
import { hasButtonPermission } from "@/hooks/useButtonPermissions";

export default function TableDocumentTrackings(props) {
    const {
        dataSource,
        onChangeTable,
        tableFilter,
        ...
        selectedRowKeys,
        setSelectedRowKeys,
    } = useContext(TabDocumentTrackingsContext);
    ...
    const hasViewPermission = useMemo(() => {
        return hasButtonPermission('M-06-TRACKINGS-VIEW');
    }, []);

    const { mutate: mutateDeactivateProfile, isLoading: loadingDeactivateProfile } =
        POST(`api/profile_deactivate`, "document_trackings_list");
