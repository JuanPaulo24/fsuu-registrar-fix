...
const styles = StyleSheet.create({
  page: {
    paddingTop: 155, // Increased to ensure proper spacing with expanded header
    paddingBottom: 72,
    ...
    pageWithRepeatingHeader: {
      paddingTop: 170, // Increased to match expanded header
      paddingRight: 50,
...
export function GeneratedDocument({ documentData = {}, profileData = {}, documentType = "Transcript of Records", qrCodeData = null }) {
  ...
  // Get formatted registrar name from backend
  const registrarDisplayName = documentData?.registrar_name || 'Registrar Name';
  
  // Get formatted staff name from backend
  const staffDisplayName = documentData?.staff_name || 'Staff Name';

  // Helper function to calculate individual page serial number
  const getPageSerialNumber = (serialRange, pageNumber) => {
    if (!serialRange) return '000000';
    
    // Check if it's a range (e.g., "000001-000003") or single number (e.g., "000001")
    if (serialRange.includes('-')) {
      const [startSerial, endSerial] = serialRange.split('-');
      const startNum = parseInt(startSerial, 10);
      const pageSerial = startNum + pageNumber - 1;
      return pageSerial.toString().padStart(6, '0');
    } else {
  ...
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <View fixed style={{ 
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 30 }}>
            <Image 
              src="/images/logo.png" 
              style={{ width: 80, height: 80, marginRight: 12, marginLeft: 12, top: 6 }}/>
            
            <View style={{ flex: 1, top: 19, left: 18 }}>
              <Text style={{ fontSize: 26, fontWeight: 'bold', right: 10,  marginBottom: 1, fontFamily: 'UniversityCyrillic', textAlign: 'left' }}>FATHER SATURNINO URIOS UNIVERSITY</Text>
              <Text style={{ fontSize: 8, marginBottom: 4, right: 10, bottom: 5, textAlign: 'left' }}>(FORMERLY URIOS COLLEGE)</Text>
              <Text style={{ fontSize: 10, marginBottom: 4, right: 10, bottom: 1, textAlign: 'left' }}>OFFICE OF THE REGISTRAR</Text>
              <Text style={{ fontSize: 7, marginBottom: 1, right: 10, bottom: 5, textAlign: 'left', lineHeight: 1.1 }}>
              ...
              <Text style={{ fontSize: 7, marginBottom: 1, right: 10, left: 150, bottom: 55, textAlign: 'left', lineHeight: 1.1 }}>
        ...
        <View fixed style={{ position: 'absolute', top: 170, right: 72 }}>
          <Text
            style={{ fontSize: 9 }}
       ...
       <View style={{ marginBottom: 10, right: 40  }}>
        ...
        <View style={{ marginBottom: 10, right: 40,}}>
          {/* Section Headers */}
          <View style={{ flexDirection: 'row', borderTop: '1pt solid black', width: '117%', paddingVertical: 2, marginBottom: 4 }}>
            <View style={{ flex: 1 }}>
        ...
        <View style={{ marginBottom: 10, right: 20 }}>
          {/* Section Header */}
          <View style={{ borderTop: '1pt solid black', width: '117%', right: 20, paddingVertical: 2, marginBottom: 4, fontSize: 9 }}>
        ...
        {/* Credit System Note */}
        <View style={{ marginBottom: 14, right: 20 }}>
          <Text style={{ fontSize: 10, textAlign: 'left', right: 20, bottom: 2, width: '117%' }}>One Unit of Credit is one hour lecture or recitation and three hours of drafting laboratory or shop work is equivalent to one hour recitation or lecture.</Text>
          <View style={{ borderTop: '1pt solid black', width: '117%', right: 20, marginTop: 1 }} />
        </View>
        {/* Footer with QR Code and Signature Areas (hidden on last page) */}
        <View
          style={{ position: 'absolute', bottom: 100, left: 51, right: 30 }}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <View style={{ alignItems: 'center', marginRight: 28 }}>
                  <View style={{ height: 80, justifyContent: 'flex-end', alignItems: 'center' }}>
                    <Text style={{ fontSize: 8 }}>{registrarDisplayName}</Text>
                    <View style={{ width: 140, borderBottom: '0.4pt solid black', marginTop: 4 }} />
                  </View>
                  <Text style={{ fontSize: 8, marginTop: 2 }}>University Registrar</Text>
                </View>
              </View>
            )
          )}
        ...
        <View style={{ position: 'absolute', bottom: 8, right: -16, fontSize: 8, color: 'red', marginRight: 40, marginBottom: 10 }}>
          <Text
            ...
            render={({ pageNumber }) => 
              `NO. ${getPageSerialNumber(documentData?.serial_number || qrCodeData?.serial_number || '000000', pageNumber)}`
            }
          />
        </View>
      </Page>
      ...
      <Page size="LETTER" style={[styles.page, styles.pageWithRepeatingHeader]}>
        ...
        <View fixed style={{ position: 'absolute', top: 170, left: 40, right: 40 }}>
          <View style={{ flexDirection: 'row', marginBottom: 2 }}>
            <Text style={{ width: 88, fontSize: 10}}>Student Name</Text>
            <Text style={{ width: 6, fontSize: 10}}>:</Text>
            <Text style={{ flex: 1, fontSize: 8, minWidth: 0, top: 2, fontWeight: 'bold'}}>{studentName}</Text>
        ...
        <View style={{ flexDirection: 'row', marginBottom: 4, marginTop: 4, top: 38, width: '115%', right: 32,alignItems: 'center' }}>
          ...
          {Array.isArray(profileData?.grades) && firstCombo && profileData.grades
            ...
            .map((g, idx) => (
              <View key={`code-row-${idx}`} style={{ flexDirection: 'row', marginBottom: 2, top: 38, width: '115%', right: 33, alignItems: 'center' }}>
                <Text style={{ width: 6, fontSize: 10 }}>: </Text>
          ...
          {remainingCombos.map((combo, comboIdx) => (
            <View key={`combo-section-${comboIdx}`} wrap>
              ...
              {Array.isArray(profileData?.grades) && profileData.grades
                ...
                .map((g, idx) => (
                  <View key={`combo-${comboIdx}-row-${idx}`} style={{ flexDirection: 'row', marginBottom: 2, top: 38, width: '115%', right: 33, alignItems: 'center' }}>
                    <Text style={{ width: 6, fontSize: 10 }}>:</Text>
                    <Text style={{ width: 85, fontSize: 10 }}>{g?.subject?.subject_code || 'N/A'}</Text>
                    <Text style={{ width: 6, fontSize: 10 }}>:</Text>
                    <Text style={{ width: 240, fontSize: 10, marginLeft: 5 }}>{g?.subject?.subject_name || 'N/A'}</Text>
          ...
          <View style={{ marginBottom: 4, top: 38, right: 32 }}>
            <View style={{ width: '108.5%', borderBottom: '1pt solid black' }} />
            <View style={{ height: 2 }} />
            <View style={{ width: '108.5%', borderBottom: '1pt solid black' }} />
          </View>
          <View style={{flexDirection: 'row', alignItems: 'center', top: 40, width: '113%', right: 33, marginBottom: 8, marginTop: 8}}>
            <Text style={{ fontSize: 9, marginRight: 18 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', top: 40, width: '108%', right: 33, marginBottom: 8, marginTop: 8 }}>
            <View style={{ flex: 1, borderBottom: '1pt dashed black' }} />
            <Text style={{ fontSize: 9, marginHorizontal: 8 }}>Nothing follows</Text>
            <View style={{ flex: 1, borderBottom: '1pt dashed black' }} />
          </View>

          {/* Footer section - Document endpoint - positioned near "Nothing follows" */}
          <View style={{ bottom: 4, left: -40, right: -40, width: '110%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 20 }}>
              <View style={{ fontSize: 8, left: '3%', textAlign: 'center', bottom: 3 }}>
                <Text>Not valid without</Text>
                <Text style={{ marginTop: 2, left: 9 }}>FSUU Seal</Text>
              </View>
              
              <View style={{ alignItems: 'center', marginRight: 28, bottom: 3}}>
                 <Text style={{ fontSize: 8, left: '40%', }}>Prepared & checked by :</Text>
                <Text style={{ fontSize: 8, marginTop: 2 ,left: '20%'}}> </Text>
              </View>

              <View style={{ marginRight: 28 }}>
                <View style={{ height: 80, justifyContent: 'flex-end', alignItems: 'flex-start' }}>
                  <Text style={{ fontSize: 8 }}>{staffDisplayName}</Text>
                  <View style={{ width: 100, borderBottom: '0.4pt solid black', marginTop: 4 }} />
                </View>
                <Text style={{ fontSize: 8, marginTop: 2 }}>Registrar Staff</Text>
              </View>

              <View style={{ alignItems: 'center', marginLeft: 28, left: '-5%', bottom: 15}}>
                <View style={{ height: 80, justifyContent: 'flex-end', alignItems: 'center' }}>
                  <Text style={{ fontSize: 8 }}>{registrarDisplayName}</Text>
                  <View style={{ width: 100, borderBottom: '0.4pt solid black', marginTop: 4 }} />
                </View>
                <Text style={{ fontSize: 8, left: '-3%', bottom: -5}}>University Registrar</Text>
              </View>
            </View>
            
            {/* Date Generated */}
            <View style={{ marginTop: 5, fontSize: 8, color: '#666666',left: '17%',marginLeft: 38 }}>
              <Text>Date Generated: {dayjs().format('MM/DD/YYYY')}</Text>
            </View>
          </View>
        ...
        <View
          fixed
          style={{ position: 'absolute', bottom: 61, left: 51, right: 30 }}
          render={({ pageNumber, totalPages }) => (
            pageNumber === totalPages ? null : (
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <View style={{ fontSize: 8, textAlign: 'center', top: 14 }}>
...
export function TranscriptOfRecordsPDFViewer({ documentData, profileData, qrCodeData }) {
  ...
  return (
    <PDFViewer style={{ width: '100%', height: '100%', border: 'none', borderRadius: 4 }} showToolbar={true}>
      <GeneratedDocument documentData={documentData} profileData={profileData} documentType={docType} qrCodeData={qrCodeData} />
    </PDFViewer>
  );
}

// Component for just the download button
export function TranscriptOfRecordsPDFDownload({ documentData, profileData, qrCodeData }) {
  const docType = documentData?.doc_category || 'Transcript of Records';
  ...
  return (
    <PDFDownloadLink 
      document={<GeneratedDocument documentData={documentData} profileData={profileData} documentType={docType} qrCodeData={qrCodeData} />} 
      fileName={filename}
...
export default function TranscriptOfRecordsPDF({ documentData, profileData, qrCodeData }) {
  const docType = documentData?.doc_category || 'Transcript of Records';
  const today = dayjs().format('YYYY-MM-DD');
  const filename = `${docType.replace(/\s+/g, '_')}_${(documentData?.full_name || profileData?.fullname || 'Student').replace(/\s+/g, '_')}_${today}.pdf`;
